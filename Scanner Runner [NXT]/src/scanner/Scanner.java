...
public class Scanner {
	...
	private static ThreadStop emergencyStop = new ThreadStop();	// Arrêt lors de l'appui sur un bouton
	...
	private static  boolean termine = false;	// Cartographie terminée / ligne d'arrivée trouvé ?
		...
		{		
			...
			{		
				...
				switch (receive.readByte()) {
					...
					default:				System.out.println("Réponse inattendue"); break;
				}
			} 
			else // termine == true
			{	
				...
				switch (receive.readByte()) {
					case STOP :		System.out.println("Fin scanner");			// Arrêt total
									break;				
					case SEQUENCE : receiveAndDoSequence();						// Récup séquence case manquante
									termine = false;
									break;
					default:		System.out.println("Réponse inattendue");	// Arrêt total également								
									break;	
				}
			}
	...
	/**
	 * Recois une séquence et la place dans une liste.
	 * Puis effectue les actions jusqu'à que la liste soit vide.
	 * @throws IOException
	 * @throws InterruptedException
	 */

 ...
 * et l'écartement des roues (respectivement 56 et 120 pour notre robot) 
...
public class DeplacementOp extends DifferentialPilot {
	...
	/**
	 * Virage en épingle : tourne sur lui même à 90 degrés 
	 * vers le coté donné en paramétre.
	 * @param cote
	 */
	...
	/**
	 * Tourne en faisant un vrai arc de cercle. Le rayon de l'arc
	 * est égale à l'écartement des roues. 
	 * @param cote
	 */
	 ...
	 * avant le lancement du programme la tête doit être positionné coté face !
	 * @param cote
	 */
	public void headTurnTo(int cote) {
		MOTOR_HEAD.setSpeed(MOTOR_HEAD.getMaxSpeed());
		switch(cote){
			case LEFT: 	MOTOR_HEAD.rotateTo(90);
						break;
			case RIGHT: MOTOR_HEAD.rotateTo(-90);
						break;
			case FACE:	MOTOR_HEAD.rotateTo(0);
						break;
		}
	}
	 ...
	 * Càd que les 2 roues ne s'arrêtent pas exactement en même temps et le robot prend un
	 * mauvais angle. myTravel vient corriger ce défaut.
	...
	public void recoveryAngleWithLine() {
		...
		while (it < 2) {					
			...
			if (it == 0) {
				...
				while (LIGHT_LEFT.getNormalizedLightValue() > lightLimit && LIGHT_RIGHT.getNormalizedLightValue() > lightLimit) {
					Thread.yield();
				}
				Sound.beep();
				stop();
	...
	/*public void recoveryAngle(int cote) throws InterruptedException {
	    ...
	    zprime = (-z*x)/(x-y);
	    angle = Math.asin(y/(zprime+z));	// Angle en radian
